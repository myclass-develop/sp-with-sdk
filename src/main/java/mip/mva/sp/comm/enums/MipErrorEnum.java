...
public enum MipErrorEnum {
	...
	SP_NEED_TO_APPLICATION_UPDATE(10113, "need to application update"), // 앱 업데이트 필요
	// 절차
	...
	SP_NETWORK_ERROR(10302, "network error"), // Network 오류

	SP_TRXINFO_NOT_FOUND(10302, "trxinfo not found"), // 존재하지 않는 거래정보
	SP_QR_IMAGE_CREATE_ERROR(10303, "SP qr image create error"), // SP QR 이미지 생성 오류
	SP_ALREADY_EXIST_TRXCODE(10304, "already exist trxcode"), // 이미 존재하는 거래코드
	SP_NOT_DEFINED_REQTYPE(10305, "not defined reqType"), // 정의 되지 않은 reqType 입니다.
	SP_REQID_NOT_EXIST(10306, "reqId does not exist"), // 존재하지 않는 reqId
	SP_CXID_NOT_EXIST(10307, "cxId does not exist"), // 존재하지 않는 cxId
	SP_SCHEMA_NAME_NOT_EXIST(10308, "schema_name does not exist"), // 존재하지 않는 스키마명
	SP_SVC_CODE_NOT_EXIST(10309, "svcCode does not exist"), // 존재하지 않는 서비스코드
	SP_REQ_TYPE_NOT_EXIST(10310, "reqType does not exist"), // 존재하지 않는 요청유형

	SP_CONFIG_ERROR(19001, "SP config error"), // SP 설정 오류
	SP_SDK_ERROR(19002, "SP SDK error"), // SP SDK 오류
	SP_CRTFC_ERROR(19003, "SP certification error"), // SP 인증 오류
	...
	MB_SP_AUTH_FAIL(90002, "SP authentication fail"), // SP 인증 실패
